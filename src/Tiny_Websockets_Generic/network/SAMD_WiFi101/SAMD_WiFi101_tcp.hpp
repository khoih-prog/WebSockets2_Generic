/****************************************************************************************************************************
  SAMD_WiFi101_tcp.hpp
  For SAMD21/SAMD51 boards with WiFi101 module/shield.
...
namespace websockets2_generic
{
  ...
  {
    typedef GenericEspTcpClient<WiFiClient> WiFi101TcpClient;

    class SecuredWiFi101TcpClient : public GenericEspTcpClient<WiFiSSLClient> 
    {
      public:
        void setInsecure() 
        {
    ...
    // KH, quick fix for WiFi101 port
    #define CLOSED     0
    
    class WiFi101TcpServer : public TcpServer 
    {
      public:
        WiFi101TcpServer() : server(DUMMY_PORT) {}
        ...
        {
          yield();
          // KH, to fix WiFi101_Generic => v1.5.3
          server.begin(port);
          //////
          return available();
        }
        ...
        {   
          while (available()) 
          {
            ...
            if (client)
            {         
              return new WiFi101TcpClient{client};
            }
